function plot_all(a,Tmax,Bmax,Cmax)
    ...
    figure(2);
    subplot(2,1,1);
    plot(T,a(:,8),T,a(:,K)*10000-5000,'r.');
        set(gca,'FontSize',12);grid on;

    title('CWND [byte]');    
    legend('CWND','In fast start');
    axis([0 Tmax 0 Cmax]);grid on;    
    subplot(2,1,2);
    K=100;B_ = ones(1,K)/K;
    TTT = [00 39.9 40 59.9 60 100];
    BBB = [5e6 5e6 1e6 1e6 5e6 5e6];
    plot(T,a(:,16),T,a(:,23),T,filter(B_,1,a(:,20)+a(:,27)),'k',TTT,BBB,'r');
        set(gca,'FontSize',12);grid on;

    axis([0 Tmax 0 Bmax]);grid on;
    title('Bitrate [bps]');    
    legend('Target, stream 1','Target, stream 2','Throughput');
    xlabel('T [s]');
