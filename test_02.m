function plot_all(a,Tmax,Bmax,Cmax)
    T = a(:,1);
    K = 12;
    %Tmax = 100.0;
    figure(1);
    subplot(2,1,1);
    plot(T,a(:,2),T,a(:,4));
    axis([0 Tmax 0 0.2]);grid on;
    title('OWD and OWD target [s]');
    legend('OWD','OWD target');
    subplot(2,1,2);
    plot(T,a(:,15),T,a(:,19),'r');
    title('RTP queue delay [s]');    
    legend('Video','Audio');
    axis([0 Tmax 0 0.03]);grid on;
    xlabel('T [s]');
    figure(2);
    subplot(2,1,1);
    plot(T,a(:,8),T,a(:,9),T,a(:,10),T,a(:,11),T,a(:,K)*1000);
    title('CWND [byte]');    
    legend('CWND','CWND infl. point','Max bytes in flight','Bytes in flight','In fast start');
    axis([0 Tmax 0 Cmax]);grid on;    
    subplot(2,1,2);
    plot(T,a(:,17),T,a(:,K)*1e5,T,a(:,16),T,a(:,20));
    axis([0 Tmax 0 Bmax]);grid on;
    title('Bitrate [bps]');    
    legend('Max bitrate','In fast start','Target bitrate video','Target bitrate audio');
    xlabel('T [s]');

    
    