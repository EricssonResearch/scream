...
#
    SCREAMTX1="queue ! screamtx name=\"screamtx1\" params=\"$NOSUMMARY -forceidr -ect 1 -initrate 500 -minrate 200 -maxrate 15000\" ! queue !"
    SCREAMTX1_RTCP="screamtx1.rtcp_sink screamtx1.rtcp_src !"
#
    SCREAMTX2="queue ! screamtx name=\"screamtx2\" params=\"$NOSUMMARY -forceidr -ect 1 -initrate 500 -minrate 200 -maxrate 1000\" ! queue !"
...
$VIDEOSRC ! $ENCODER name=encoder0 ! rtph264pay config-interval=-1 ! $SCREAMTX0 r.send_rtp_sink_0 r.send_rtp_src_0 ! $NETSIM udpsink host=$DST_IP port=$PORT0_RTP sync=false \
...
\
$VIDEOSRC ! $ENCODER name=encoder1 ! rtph264pay config-interval=-1 ! $SCREAMTX1 r.send_rtp_sink_1 r.send_rtp_src_1 ! $NETSIM udpsink host=$DST_IP port=$PORT1_RTP sync=false \
...
\
$VIDEOSRC ! $ENCODER name=encoder2 ! rtph264pay config-interval=-1 ! $SCREAMTX2 r.send_rtp_sink_2 r.send_rtp_src_2 ! $NETSIM udpsink host=$DST_IP port=$PORT2_RTP sync=false \
