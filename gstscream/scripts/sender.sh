...
$VIDEOSRC ! $ENCODER name=encoder0 ! rtph264pay config-interval=-1 ! $SCREAMTX0 r.send_rtp_sink_0 r.send_rtp_src_0 ! udpsink host=$DST_IP port=$PORT0_RTP sync=false $SET_ECN \
